...
1. **CPU使用特点：**
- MapReduce：CPU使用率波动剧烈(0-100%)，前期高负载，后期低负载
- Spark：CPU使用稳定在25-30%，波动小，资源利用更均衡

2. **磁盘I/O模式：**
- MapReduce：频繁的读写操作，写入峰值达到5×10^8字节，呈现大幅波动
- Spark：写入操作较少且规律，峰值仅为7×10^6字节，整体I/O负载低

3. **内存使用模式：**
- MapReduce：内存使用波动大(2-6×10^9字节)，峰值高
- Spark：内存使用平稳(稳定在2.6×10^9字节)，增长缓慢

综合原因：
1. **计算模型差异**
    - MapReduce必须将中间结果写入磁盘，导致高I/O和内存波动
    - Spark的内存计算模型减少了I/O操作，资源使用更稳定

2. **数据处理方式**
    - MapReduce的序列化/反序列化操作频繁，增加CPU和内存开销
    - Spark通过RDD机制和内存计算，减少了数据转换开销

3. **资源调度效率**
    - MapReduce的粗粒度调度导致资源使用不均衡
    - Spark的细粒度调度实现了更好的资源利用

这些差异共同导致了Spark在执行相同任务时表现出更高的性能和资源利用效率。
