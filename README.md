...
1. **CPU使用差异**
- Mapper阶段CPU使用率80-100%剧烈波动(文本分词、正则处理和序列化)，Reducer阶段降至30%趋于稳定(数据汇总和排序)，而Spark全程保持25-30%稳定使用率，体现了内存计算和流水线优化的优势。

2. **磁盘I/O差异**
- MapReduce因Mapper和Reducer阶段间数据落盘导致频繁I/O，写入峰值达5×10^8字节；而Spark通过内存计算将I/O峰值控制在7×10^6字节，大大减少了磁盘访问。

3. **内存使用差异**
- MapReduce内存使用在2-6×10^9字节间波动，用于缓存中间结果和数据序列化；Spark则稳定在2.6×10^9字节，通过RDD机制实现更高效的内存管理。

这些差异直观地展示了Spark基于内存的计算模型相比MapReduce基于磁盘的模型具有更优的资源利用效率。
## 分工
张志凡：数据集准备、实验设计、环境搭建及PPT制作，贡献度25%

郑舒岩：环境搭建、实验设计、程序编写、运行调试，贡献度25%

张文宇：实验程序编写、信息收集、运行调试，贡献度25%

王思腾：实验程序编写、信息收集、运行调试，贡献度25%
